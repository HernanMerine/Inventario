<?php
	...
	function paginador_tablas($pagina, $Npaginas, $url, $botones) {
		...
		for ($i = $pagina; $i <= $Npaginas; $i++) {
			...
			if ($pagina == $i) {
				$tabla .= '<li><a class="pagination-link is-current">'.$i.'</a></li>';
			} else {
		...
		if ($pagina == $Npaginas) {
			$tabla .= '</ul><a class="pagination-next" disabled>Siguiente</a>';
		} else {
			if ($pagina < $Npaginas - 1) {
				$tabla .= '<li><span class="pagination-ellipsis">&hellip;</span></li>';
			}
			$tabla .= '<li><a class="pagination-link" href="'.$url.$Npaginas.'">'.$Npaginas.'</a></li></ul>';
